...
Mat Mat::Get(int row_start, int row_size, int col_start, int col_size)
{
    ...
    {
        memcpy(&result.data[r * result.cols], &this->data[(r + row_start) * this->stride + col_start],
...
Mat& Mat::operator=(const Mat& m)
{
    ...
    if (this->rows != m.rows || this->cols != m.cols)
    {
        ...
        {
            ESP_LOGE("Mat",
                "operator = Error for sub-matrices: operands matrices dimensions %dx%d and %dx%d do not match",
...
Mat& Mat::operator+=(const Mat& m)
{
    ...
    {
        dspm_add_f32(this->data, m.data, this->data, this->rows, this->cols, this->padding, m.padding, this->padding, 1,
            1, 1);
    }
    else
    {
...
Mat& Mat::operator-=(const Mat& m)
{
    ...
    if (this->sub_matrix || m.sub_matrix)
    {
        dspm_sub_f32(this->data, m.data, this->data, this->rows, this->cols, this->padding, m.padding, this->padding, 1,
            1, 1);
    }
    else
    {
...
Mat& Mat::operator*=(const Mat& m)
{
    ...
    if (this->sub_matrix || m.sub_matrix)
    {
        ...
        dspm_mult_ex_f32(temp.data, m.data, this->data, temp.rows, temp.cols, m.cols, temp.padding, m.padding,
            this->padding);
    }
    else
    {
